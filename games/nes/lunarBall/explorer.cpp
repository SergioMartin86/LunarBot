...
#define _MAX_STROKES 9
#define _ANGLE_COUNT 256
#define _INITIAL_MAX_FRAMES 10000
#define _MAX_NEW_CANDIDATES_PER_STROKE 500
#define _STATE_SIZE 2219ul
...
struct candidate_t
{
  ...
  uint32_t _angleFrames[_MAX_STROKES];
  uint32_t _extraAngleFrames[_MAX_STROKES];
  uint32_t _speedFrames[_MAX_STROKES];
  uint32_t _playFrames[_MAX_STROKES];
  uint32_t transitionFrames[_MAX_STROKES];
...
int main(int argc, char *argv[])
{
  ...
  if (stateSize != _STATE_SIZE) { fprintf(stderr, "State Size mismatch: %lu vs %lu\n", stateSize, _STATE_SIZE); exit(-1); }
  ...
  if (currentStage == 60) if (currentStage != _CURRENT_STAGE) { fprintf(stderr, "Wrong stage selected %u vs %u\n", currentStage, _CURRENT_STAGE); exit(1); }
  ...
  // if (_winCandidatesFound == 0) // Used for min strokes 
  {
    ...
    while (_nextCandidateMoves.begin() != _nextCandidateMoves.end())
    {
      ...
      jaffarCommon::logger::log("[J+] Processing entry %lu / %u with reward: %f - Remaining Balls: %u - Frame Count: %u\n", currentCandidateCounter, _MAX_NEW_CANDIDATES_PER_STROKE, srcCandidateReward, srcCandidatePtr->_pendingBalls, srcCandidatePtr->_frameCount);
      for (size_t i = 0; i < 256; i++)
       for (size_t j = 255; j >= 24; j -= 3)
        #ifdef _CONSIDER_SPEED_INCREMENT
    ...
    JAFFAR_PARALLEL
    {
      ...
      while(_currentCandidateMoves.try_pop(candidate) == true)
      {
        ...
        if (candidate->_frameCount < (_maxFrames - _JINGLE_TIME))
        {
          ...
          #endif
          if (*g->_ball0Angle != targetAngle) { fprintf(stderr, "Wrong Angle %u vs %u\n", *g->_ball0Angle, targetAngle); JAFFAR_THROW_RUNTIME("Wrong angle"); }
          if (*g->_ball0Speed != targetSpeed) JAFFAR_THROW_RUNTIME("Wrong speed");

          // Launching the ball (pressing B button until state changes)
          g->advanceState(g->_buttonInputIdx); candidate->_frameCount++;
          g->advanceState(g->_buttonInputIdx); candidate->_frameCount++;
        }
        ...
        while(collisionFound == false && candidate->_frameCount < (_maxFrames - _JINGLE_TIME))
        {
          ...
          if (currentFrame % 50000 == 0) 
          {
            ...
            jaffarCommon::logger::log("[J+] Current Frame: %lu - Angle: %u, Power: %u, Hash: %s\n",  currentFrame, candidate->_angle[currentStroke], candidate->_speed[currentStroke], hashString.c_str());
          }
          ...
          if (g->_movingBalls == 0) { isFinite = true; break; }
        }

        // If cue ball was inserted, reset pending balls now
        if (*g->_ball0State == jaffarPlus::games::nes::LunarBall::ballState_t::entering || *g->_ball0State == jaffarPlus::games::nes::LunarBall::ballState_t::entered) g->_pendingBalls = initialPendingBalls;
